...
class DateFormat {
    constructor(date) {
        this.dd = (date.getDate()).toLocaleString('en-US', {minimumIntegerDigits: 2, useGrouping:false});
        ...
        this.month_int = (parseInt(date.getMonth() + 1))
    ...
    format(pattern) {
        ...
        pattern = (pattern.indexOf("d") > -1) ?
            ...
            pattern;
        pattern = (pattern.indexOf("MM") > -1) ?
            pattern.replace(new Array(pattern.match(/M/g).length + 1).join('M'), (pattern.match(/M/g).length > 2) ? (pattern.match(/M/g).length > 3) ? this.month : this.month.substring(0, 3) :
                this.month_int.substring(0, 2)) : pattern;
        pattern = (pattern.indexOf("D") > -1) ?
            pattern.replace(new Array(pattern.match(/D/g).length + 1).join('D'), (pattern.match(/D/g).length > 2) ? this.day :
                this.day.substring(0, 3)) : pattern;
        return pattern;
    };
}
